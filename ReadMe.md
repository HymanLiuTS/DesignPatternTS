...
（2）远程代理：远程代理可以作为另一个JVM上对象的本地代表，调用代理的方法，会被代理利用网络转发到远程执行，并且结果会通过网络返回给代理，再由代理将结果转给客户。<br>
（3）虚拟代理:虚拟代理经常直到我们真正需要一个对象的时候才创建它,当对象在创建前和创建中时，由虚拟代理来扮演对象的替身。对象创建后，代理就会将请求直接委托给对象。例如我们在web 上加载某个图片时，加载过程耗时较长，这时我们可以使用图片的虚拟代码，进行一些加载过程提示信息的显示<br>
虚拟代理模式：<br>
![avatar](http://123.207.107.215/images/designpattern/virtualproxypattern.jpg)<br>
（4）保护（动态）代理：可以控制对对象的访问（控制对象暴露哪些接口，隐藏哪些接口），在程序运行时被创建。在Java中使用动态代理时，只需实现InvocationHandler接口，动态代理Proxy已经在java中定义。
![avatar](http://123.207.107.215/images/designpattern/dynamicproxy.jpg)<br>
（5）防火墙代理：控制网络资源的访问，保护主体免于坏客户的侵害<br>
（6）智能引用代理：当主题被引用时，进行额外的动作，比如计算一个对象被引用的次数<br>
（7）缓存代理：为开销大的运算结果提供暂时的存储，它也允许多个客户共享结果，以减少计算或者网路延迟<br>
（8）同步代理：在多线程的情况下为主题提供安全的访问<br>
（9）复杂隐藏代理：用来隐藏一个类的复杂集合的复杂度，并进行访问控制。有时候也称作外观代理。<br>
（10）写入时复制代理：用来控制对象的复制，方法是延迟对象的复制，直到客户真的需要为止。<br>
