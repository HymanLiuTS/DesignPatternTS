...
（6）智能引用代理：当主题被引用时，进行额外的动作，比如计算一个对象被引用的次数
（7）缓存代理：为开销大的运算结果提供暂时的存储，它也允许多个客户共享结果，以减少计算或者网路延迟
（8）同步代理：在多线程的情况下为主题提供安全的访问
（9）复杂隐藏代理：用来隐藏一个类的复杂集合的复杂度，并进行访问控制。有时候也称作外观代理。
